...
public class ClockWidgetService extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		if (intent != null
				&& ACTION_REFRESH_CALENDAR.equals(intent.getAction())) {
			if (D)
				Log.v(TAG, "Forcing a calendar refresh");
	...
	/**
	 * Reload the widget including the Weather forecast, Alarm, Clock font and
	 * Calendar
	 */
	private void refreshWidget() {
		...
		for (int id : mWidgetIds) {
			...
			if (showCalendar) {
				boolean canFitCalendar = WidgetUtils.canFitCalendar(this, id,
						digitalClock);
				remoteViews.setViewVisibility(R.id.calendar_panel,
						canFitCalendar ? View.VISIBLE : View.GONE);
			}
	...
	private void refreshClock(RemoteViews clockViews, boolean digitalClock) {
		...
		Intent i = new Intent(Intent.ACTION_MAIN).addCategory(
				Intent.CATEGORY_LAUNCHER).setComponent(clk);
	...
	private void setClockSize(RemoteViews clockViews, float scale) {
		float fontSize = getResources().getDimension(
				R.dimen.widget_big_font_size);
		clockViews.setTextViewTextSize(R.id.clock1_bold,
				TypedValue.COMPLEX_UNIT_PX, fontSize * scale);
		clockViews.setTextViewTextSize(R.id.clock1_regular,
				TypedValue.COMPLEX_UNIT_PX, fontSize * scale);
		clockViews.setTextViewTextSize(R.id.clock2_bold,
				TypedValue.COMPLEX_UNIT_PX, fontSize * scale);
		clockViews.setTextViewTextSize(R.id.clock2_regular,
				TypedValue.COMPLEX_UNIT_PX, fontSize * scale);
	}
	...
	/**
	 * @return A formatted string of the next alarm or null if there is no next
	 *         alarm.
	 */
	...
	private boolean refreshCalendar(RemoteViews calendarViews) {
		...
		if (!mCalendarInfo.hasEvents() || mRefreshCalendar) {
			if (D)
				Log.d(TAG, "Checking for calendar events...");
			getCalendarEvents(lookAhead, calendarList, remindersOnly,
					hideAllDay);
			mRefreshCalendar = false;
		}
		...
		for (EventInfo event : mCalendarInfo.getEvents()) {
			final RemoteViews itemViews = new RemoteViews(getPackageName(),
					R.layout.calendar_item);

			// Only set the icon on the first event
			if (!hasEvents) {
				itemViews.setImageViewResource(R.id.calendar_icon,
						R.drawable.ic_lock_idle_calendar);
			}

			// Add the event text fields
			itemViews.setTextViewText(R.id.calendar_event_title, event.title);
			itemViews.setTextViewText(R.id.calendar_event_details,
					event.description);

			if (D)
				Log.v(TAG, "Showing event: " + event.title);
	...
	private void getCalendarEvents(long lookahead, Set<String> calendars,
		...
		try {
			...
			if (cursor != null) {
				...
				final java.text.DateFormat eventFormat = DateFormat
						.getTimeFormat(this);
				final int showLocation = Preferences.calendarLocationMode(this);
				final int showDescription = Preferences
						.calendarDescriptionMode(this);
				...
				while (cursor.moveToNext()
						&& eventCount < Constants.MAX_CALENDAR_ITEMS) {
					...
					if (D)
						Log.v(TAG, "Adding event: " + title + " with id: "
								+ eventId);
					...
					if (showLocation != Preferences.SHOW_NEVER
						...
						case Preferences.SHOW_FIRST_LINE:
							...
							if (stringEnd == -1) {
								...
								sb.append(": "
										+ location.substring(0, stringEnd));
							}
							break;
					...
					// to next record
					mCalendarInfo.addEvent(populateEventInfo(eventId, title,
							sb.toString(), begin, end, allDay));
					eventCount++;
				}
			}
		} catch (Exception e) {
	 ...
	 */
	private EventInfo populateEventInfo(long eventId, String title,
			String description, long begin, long end, boolean allDay) {
	...
	private long calculateUpdateTime() {
		...
		if (mCalendarInfo.getFollowingEventStart() > 0) {
			...
			minUpdateTime = Math.min(minUpdateTime,
					mCalendarInfo.getFollowingEventStart() - lookAhead);
		}
		...
		if (D) {
			...
			Log.i(TAG,
					"Chronus: It is now "
							+ DateFormat.getTimeFormat(this).format(date1)
							+ ", next widget update at "
							+ DateFormat.getTimeFormat(this).format(date2));
		}
